...
function openModal(id) {
  if (id) {
    ...
    state.product = productStore.store.products.find((item) => item._id === id);
  } else {
...
<template>
  <div id="page-content-wrapper" class="p-3">
    ...
    <table class="table table-striped">
      ...
      <tbody>
        <tr v-for="(product, index) in store.products" :key="product._id">
          <td>{{ index + 1 }}</td>
          <td>{{ product.title }}</td>
          <td>{{ product.price }}</td>
          ...
          <td class="text-center">
            <a @click="openModal(product._id)" href="#" class="me-3"><PencilSvg /></a>
            <a @click="deleteProduct(product._id)" href="#"><TrashSvg /></a>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <ModalComponent id="modal" @modal-close="closeModal">
      ...
      <template #content><FormProduct :product="state.product" @modal-close="closeModal" /></template>
    </ModalComponent>
  </div>
</template>
